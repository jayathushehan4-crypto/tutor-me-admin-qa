/**
 * Edexcel A/L Past Paper Uploader
 * Reads manifest.json files created by scrape-edexcel-al-papers.mjs,
 * uploads each PDF to Azure Blob Storage, then creates paper records
 * under the "Edexcel Advanced Level" grade.
 *
 * Prerequisites:
 *   1. node scripts/scrape-edexcel-al-papers.mjs
 *   2. node scripts/add-edexcel-al-subjects.mjs <email> <password>
 *   3. AZURE_STORAGE_KEY='<key>' (bash) or $env:AZURE_STORAGE_KEY='<key>' (PowerShell)
 *
 * Usage: node scripts/upload-edexcel-al-papers.mjs <email> <password>
 */
...
const PAPERS_DIR  = process.env.PAPERS_DIR ?? "D:/Download/Edexcel-AL-Papers";

const AZURE_ACCOUNT   = "tutormeuploads";
const AZURE_CONTAINER = "uploads";
const AZURE_KEY       = process.env.AZURE_STORAGE_KEY ?? "";

const GRADE_TITLE = "Edexcel Advanced Level";
...
async function main() {
  console.log("=== Edexcel A/L Paper Uploader ===\n");

  if (!AZURE_KEY) {
    console.error("AZURE_STORAGE_KEY env variable is not set.");
    console.error("Run: AZURE_STORAGE_KEY='your-key' node scripts/upload-edexcel-al-papers.mjs");
    process.exit(1);
  }
  ...
  if (!grade) throw new Error(`Grade "${GRADE_TITLE}" not found. Run add-edexcel-al-subjects.mjs first.`);
  ...
  if (!fs.existsSync(PAPERS_DIR)) {
    throw new Error(`Downloaded papers folder not found: ${PAPERS_DIR}. Run scrape-edexcel-al-papers.mjs first.`);
  }
  ...
  if (subjectFolders.length === 0) {
    console.log("No manifest.json files found. Run scrape-edexcel-al-papers.mjs first.");
    return;
  }
  ...
  for (const folderName of subjectFolders) {
    ...
    if (!subjectId) {
      ...
      console.log(`   → Run add-edexcel-al-subjects.mjs first to add missing subjects.`);
