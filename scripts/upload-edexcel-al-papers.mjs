...
async function main() {
  ...
  if (!AZURE_KEY) {
    ...
    console.error(
      "Run: AZURE_STORAGE_KEY='your-key' node scripts/upload-edexcel-al-papers.mjs",
    );
    process.exit(1);
  }
  ...
  if (!grade)
    throw new Error(
      `Grade "${GRADE_TITLE}" not found. Run add-edexcel-al-subjects.mjs first.`,
    );
  console.log(
  ...
  if (!fs.existsSync(PAPERS_DIR)) {
    throw new Error(
      `Downloaded papers folder not found: ${PAPERS_DIR}. Run scrape-edexcel-al-papers.mjs first.`,
    );
  }
  ...
  if (subjectFolders.length === 0) {
    console.log(
      "No manifest.json files found. Run scrape-edexcel-al-papers.mjs first.",
    );
    return;
  }
  ...
  for (const folderName of subjectFolders) {
    ...
    if (!subjectId) {
      ...
      console.log(
        `   → Run add-edexcel-al-subjects.mjs first to add missing subjects.`,
      );
